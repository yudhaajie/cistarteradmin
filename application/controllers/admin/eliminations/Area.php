...
class Area extends DZ_Controller
{
    ...
    {
        return $this->load->view('admin/elimination/area_registrant');
    }
    ...
    public function loadTable()
    {
        ...
        $table        = "registration";
        $condition    = [ 'registration.deleted_date IS NULL', 'registration_payment.status > 1' ];
        $row          = [
            '[number]',
            'UPPER(registration.name)',
            'UPPER(regional_area.name) as area',
            'UPPER(regional.name) as regional',
            'CASE(registration_payment.status)
                WHEN 2 THEN "AREA QUALIFICATION STAGE"
                WHEN 3 THEN "GAGAL"
                ELSE "LOLOS"
                END as status',
            'registration.id'
        ...
        $row_search   = [
            'registration.name', 'regional_area.name as area', 'regional.name as regional',
            'CASE(registration_payment.status)
                WHEN 2 THEN "AREA QUALIFICATION STAGE"
                WHEN 3 THEN "GAGAL"
                ELSE "LOLOS"
    ...
    public function update_status($id, $status)
    {
        ...
        else
        {
            ...
            $teamName   = $team->name;
            $statuss    = ($status == 3) ? "GAGAL" : "LOLOS";
            $responsnya = " {$teamName} - {$statuss} - AREA - {$name} - {$ip_address}";
            $responLog  = date("Y-M-d H:i:s") . $responsnya . PHP_EOL;

            file_put_contents('./documents/log_eliminasi_area.txt', $responLog, FILE_APPEND);
