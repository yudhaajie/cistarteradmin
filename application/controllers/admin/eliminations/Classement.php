...
//index
    ...
    {
        $result = $this->model_clasement->district_clasements(39);
        $data['clasements'] = $result['clasements'];
    ...
    public function store_clasement()
    {
        if ($this->input->method() == "post") {
            ...
            $clasement_id =  $this->model_clasement->insert($data);
            //insert team
            $result = '';
            foreach ($teams as $id) {
                $data = array();
                ...
                $data['created_date'] = $date;
                $data['created_by'] = $userdata->user;
                $result .= $this->model_clasement->insert_team($data);
            }
            if (!$result)
    ...
    {
        $data['clasement'] = $this->model_clasement->get_clasement($id);
        ...
        $data['formats'] = $this->model_match->match_formats();
        $data['clasement_teams'] = $this->model_clasement->clasement_teams($id);
        $data['teams'] = $this->model_clasement->free_teams($district_id);
        $data['id']     = $id;
    ...
    public function update_clasement($id)
    {
        if ($this->input->method() == "post") {
            ...
            $get    = $this->model_clasement->get_clasement($id);
            ...
            if ($get) {
                ...
                $result = $this->model_clasement->update($id, $data);
                $result = '';
                $team = $this->model_clasement->clasement_teams($id);
                foreach ($teams as $team_id) {
                    echo $team_id;
                }
                // exit();
                if($team and $teams!=[])  $delete = $this->model_clasement->delete_team($id);
    ...
    {
        $userdata = $this->session->user('admin');
        $result   = $this->model_clasement->delete($id, $userdata->user);
        $delete = $this->model_clasement->delete_team($id);
