...
class Regional extends DZ_Controller
{
    ...
    {
        return $this->load->view('admin/elimination/regional_registrant');
    }
    ...
    public function loadTable()
    {
        ...
        $table        = "registration";
        $condition    = [ 'registration.deleted_date IS NULL', 'registration_payment.status > 3' ];
        $row          = [
            '[number]',
            ...
            'CASE(registration_payment.status)
                WHEN 4 THEN "REGIONAL QUALIFICATION STAGE"
                WHEN 5 THEN "GAGAL"
                ELSE "LOLOS"
                END as status',
            'registration.id'
    ...
    public function update_status($id, $status)
    {
        ...
        else
        {
            ...
            $teamName   = $team->name;
            $statuss    = ($status == 5) ? "GAGAL" : "LOLOS";
            $responsnya = " {$teamName} - {$statuss} - REGIONAL - {$name} - {$ip_address}";
            $responLog  = date("Y-M-d H:i:s") . $responsnya . PHP_EOL;

            file_put_contents('./documents/log_eliminasi_regional.txt', $responLog, FILE_APPEND);
